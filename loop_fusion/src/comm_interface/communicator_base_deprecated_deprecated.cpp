...
auto CommunicatorBase::CheckBuffer()->bool {
    send_buf_.clear();
    ...
    while(CheckBuffer()){
        ...
        if(msg_type_deserialize_[1] == 0) { // new data msg
            if(msg_type_deserialize_[4] == 0) { // Keyframe
                MsgKeyframeBase msg(msg_type_deserialize_);
                iarchive(msg);
                buffer_keyframes_in_.push_back(msg);
//                std::cout << "Msg KF " << msg.id << ": \n" << " KF ref: " << msg.id_reference << "\n" << "T_sred_s: \n" << msg.T_sref_s << std::endl;
//                std::cout << "Msg KF " << msg.id << std::endl;
            } else if(msg_type_deserialize_[4] == 1) { // Landmark
                MsgLandmarkBase msg(msg_type_deserialize_);
                iarchive(msg);
                buffer_landmarks_in_.push_back(msg);
//                cnt_lms_total++;
            } else {
                std::cout << "ERROR: " << "msg_type_deserialize[4]: " << msg_type_deserialize_[4] << std::endl;
                exit(-1);
            }
            send_buf_.clear();
        } else if(msg_type_deserialize_[1] == 1) { // update msg
            std::cout << "--> not processing updates currently!!!" << std::endl;
            send_buf_.clear();
        } else {
            std::cout << "ERROR: " << "msg_type_deserialize[1]: " << msg_type_deserialize_[1] << std::endl;
            exit(-1);
        }
//        if(cntx%20==0) outx = ".";
//        else outx += ".";
...
//    if(cnt_lms_total) std::cout << "Landmarks received total: " << cnt_lms_total << std::endl;
}
...
auto CommunicatorBase::ProcessBufferOut()->void {
    ...
    while(!buffer_data_out_.empty()){
        ...
        for(int i=0;db.keyframes.size();++i){
            ...
            MsgKeyframeBase msg = db.keyframes.front();
...
//            int cnt_lms_it = 0;
            ...
            for(int i = 0; i<ContainerSize; i++){
                if(!db.landmarks.empty()){
                    MsgLandmarkBase msg = db.landmarks.front();
...
//            std::cout << "Landmarks sent this iteration: " << cnt_lms_it << std::endl;
        }
//        std::cout << "Landmarks sent total: " << cnt_lms_total << std::endl;
    }
}
...
//        std::cout << "Iterate recv" << std::endl;
    }
}
...
auto CommunicatorBase::Serialize(MsgKeyframeBase &msg)->void {
    //clear stringstream
    send_ser_.str("");
    send_ser_.clear();
...
auto CommunicatorBase::Serialize(MsgLandmarkBase &msg)->void {
    //clear stringstream
    send_ser_.str("");
    send_ser_.clear();
...
auto CommunicatorBase::unpacki32(std::vector<unsigned char> &buf, MsgInfoType &msg)->void {
    ...
    for(int i = 0; i < buf.size()/sizeof(msg[0]); i++)
